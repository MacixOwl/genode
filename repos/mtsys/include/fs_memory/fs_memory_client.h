// Combine Fs client and memory client into one client

#ifndef _INCLUDE__FS_MEMORY_CLIENT_H_
#define _INCLUDE__FS_MEMORY_CLIENT_H_
...
struct MtsysFsMemory::Session_client : Genode::Rpc_client<Session>
{
    ...
    int read(int fd, Genode::addr_t buf, Genode::size_t count) override
    {
        return call<Rpc_read>(fd, buf, count);
    }

    int write(int fd, Genode::addr_t buf, Genode::size_t count) override
    {
...
#endif /* _INCLUDE__FS_MEMORY_CLIENT_H_ */

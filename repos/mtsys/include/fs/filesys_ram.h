...
#ifndef _INCLUDE__RAM_FILE_SYSTEM_H_
#define _INCLUDE__RAM_FILE_SYSTEM_H_
...
struct Mfs_ram::Io_handle final : public  Vfs_handle,
...
struct Mfs_ram::Watch_handle final : public  Vfs_watch_handle,
...
class Mfs_ram::Node : private Genode::Avl_node<Node>
{
	private:
	...
	public:
		...
		{
			Genode::error("Mfs_ram::Node::write() called");
			return 0;
		}

		virtual void truncate(Seek)
		{
			Genode::error("Mfs_ram::Node::truncate() called");
		}
...
class Mfs_ram::File : public Mfs_ram::Node
{
	private:
...
class Mfs_ram::Symlink : public Mfs_ram::Node
{
	private:
...
class Mfs_ram::Directory : public Mfs_ram::Node
{
	private:
	...
	public:
		                                                ...
		                                                size_t &out_count) override
		{
			...
			auto dirent_type = [&] ()
			{
				if (dynamic_cast<File      *>(node_ptr)) 
					return Dirent_type::CONTINUOUS_FILE;
				if (dynamic_cast<Directory *>(node_ptr)) 
					return Dirent_type::DIRECTORY;
				if (dynamic_cast<Symlink   *>(node_ptr)) 
					return Dirent_type::SYMLINK;
...
class Mfs::Ram_file_system : public Vfs::File_system
{
	private:

		friend class Genode::List<Mfs_ram::Watch_handle>;

		// Mfs::Env           &_env;
		Genode::Env &_env;
		Genode::Allocator &_alloc;
		Mfs_ram::Directory  _root = { "" };

		Mfs_ram::Node *lookup(char const *path, bool return_parent = false)
		{
			using namespace Mfs_ram;
		...
		Mfs_ram::Directory *lookup_parent(char const *path)
		{
			using namespace Mfs_ram;
	...
	public:
		...
		Ram_file_system(Genode::Env &env, Genode::Allocator &alloc) 
		:_env(env),
		_alloc(alloc)
		{ }
		...
		{
			Mfs_ram::Io_handle * const ram_handle =
				static_cast<Mfs_ram::Io_handle *>(vfs_handle);

			Mfs_ram::Node &node = ram_handle->node;
		                   ...
		                   Allocator &alloc) override
		{
			...
			try {
				Mfs_ram::Watch_handle * const watch_handle = new(alloc)
					Mfs_ram::Watch_handle(*this, alloc, *node);
		...
		{
			Mfs_ram::Watch_handle * const watch_handle =
				static_cast<Mfs_ram::Watch_handle *>(vfs_handle);
		                   ...
		                   size_t &out) override
		{
			...
			Mfs_ram::Io_handle &handle =
				*static_cast<Mfs_ram::Io_handle *>(vfs_handle);

			Mfs_ram::Seek const seek { size_t(handle.seek()) };
		...
		Ftruncate_result ftruncate(Vfs_handle * const vfs_handle, file_size len) override
		{
			...
			Mfs_ram::Seek const at { size_t(len) };

			try { handle.node.truncate(at); }
			catch (Mfs_ram::Out_of_memory) { return FTRUNCATE_ERR_NO_SPACE; }
			return FTRUNCATE_OK;
		}
		...
		static char const *name()   { return "Mfs"; }
		char const *type() override { return "Mfs"; }
};

#endif /* _INCLUDE__RAM_FILE_SYSTEM_H_ */
